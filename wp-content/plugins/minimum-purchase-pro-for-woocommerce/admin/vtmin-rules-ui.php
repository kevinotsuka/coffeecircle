   ...
   public    function vtmin_pop_in_select( ) {
       ...
       ?>            
        ...
        <div class="column1" id="inpopDescrip">
            <h4> <?php _e('Choose how to search the cart contents', 'vtmin') ?></h4>
            <p> <?php _e('Minimum Purchase rules will only look at the contents of the cart at checkout.
            The cart can be searched based on a wide variety of criteria - by a single product id,
            by a group of products, or by all products in the cart.  Product groups can look at product categories,
            custom categories, the logged-in user membership status, or any combination.', 'vtmin') ?>           
            </p>
        </div>
       ...
       <div class="<?php //echo $groupPop_vis ?> " id="vtmin-pop-in-cntl">                                                  
         ...
         <br><br>
         Please take note of the relationship choice 'and/or' when using roles.  The default is 'or', while choosing 'and' requires that 
         both a role and a category be selected, before a rule can be published.", 'vtmin')?>
         </p> 
...
*/
  ...
  public function vtmin_build_checkbox_contents ($taxonomy, $tax_class, $checked_list = NULL) {
        ...
        foreach ($categories as $category) {
            ...
            if ($checked_list) {
                if (in_array($category['term_id'], $checked_list)) {   //if cat_id is in previously checked_list      if (in_array("Irix", $os)) {
    ...
    public function vtmin_ajax_load_variations() {
    global $wpdb, $post, $vtmin_rule;

    $vtmin_rule->inpop_varProdID['value']  = $_POST['inVarProdID'];  //from var *passed in from ajax js  
    $product_ID = $vtmin_rule->inpop_varProdID['value'];    
    $product_variation_IDs = $this->vtmin_ajax_edit_product($product_ID);
    
    if ($vtmin_rule->rule_error_message[0] > ' ') {
       echo '<div id="inVariationsError">';
       echo $vtmin_rule->rule_error_message[0];
       echo '</div>';
    } else {
       $this->vtmin_ajax_show_variations ($product_variation_IDs);
    }
  ...
  ********************************************* */
  ...
  public function vtmin_ajax_show_variations ($product_variation_IDs) {
     global $post, $vtmin_info, $vtmin_rule; $vtmin_rules_set;
     ?>             
          <h3><?php _e('Product Variations', 'vtmin')?></h3>                  
     <?php
            //********************************
            $this->vtmin_post_category_meta_box($post, array( 'args' => array( 'taxonomy' => 'variations', 'tax_class' => 'var-in', 'checked_list' => $vtmin_rule->var_in_checked, 'product_ID' => $vtmin_rule->inpop_varProdID['value'], 'product_variation_IDs' => $product_variation_IDs )));
            // ********************************                 
  } 
